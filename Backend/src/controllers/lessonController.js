...
export const createLesson = async (req, res) => {
  try {
    ...
    // AUTO-PUBLISH: nếu course đang draft → chuyển sang published
...
export const deleteLesson = async (req, res) => {
  try {
    ...
    // ── GUARD: không ai được xóa lesson cuối cùng ──
    const currentCount = await countLessons(lesson.course_id);
    if (currentCount <= 1) {
      return res.status(400).json({
        error: 'Không thể xóa bài học cuối cùng. Khóa học phải có ít nhất 1 bài học.',
      });
    }
