...
export default function CourseListPage() {
  ...
  // Load toàn bộ courses khi category thay đổi
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      setError('');
      try {
        let res: any;
        if (selectedCategory) {
          res = await courseService.getByCategory(selectedCategory, 1, 9999);
        } else {
          res = await courseService.getAll(1, 9999);
        }
  ...
  // Tính totalPages từ filteredCourses
  const totalPages = Math.max(1, Math.ceil(filteredCourses.length / PAGE_SIZE));

  // Slice ra đúng trang hiện tại
