...
import { Star, Users, Clock, ChevronRight, BookOpen, Lock, AlertTriangle } from 'lucide-react';
...
export default function CourseDetailsPage() {
  ...
  // ── Trạng thái nút enroll ────────────────────────────────────────────────
  const isArchived  = course?.status === 'archived';
  const isPublished = course?.status === 'published';
  const hasLesson   = (course?.lesson_count ?? 0) > 0;
  const canEnroll   = isPublished && hasLesson && !isArchived;

  // Student đã enrolled vào course archived → vẫn cho vào học
  const EnrollButton = () => {
    if (isEnrolled) {
      return (
        <Link
    ...
    if (isArchived) {
      return (
        <div className="mb-3">
          <div className="w-full flex items-center justify-center gap-2 bg-gray-100 text-gray-400 py-3 rounded-lg font-semibold cursor-not-allowed select-none mb-2">
            <Lock className="h-4 w-4" />
            Không thể đăng ký
          </div>
          <p className="text-xs text-center text-red-400 flex items-center justify-center gap-1">
            <AlertTriangle className="h-3 w-3" />
            Khóa học đã ngưng xuất bản
          </p>
        </div>
      );
    }
      ...
      return (
        <div className="mb-3">
          ...
          <p className="text-xs text-center text-gray-400">
            {!hasLesson ? 'Khóa học chưa có bài học nào' : 'Khóa học chưa được công bố'}
          </p>
        </div>
      );
    }
    ...
    return (
      <button
        ...
        disabled={enrolling}
        className="w-full bg-orange-500 text-white py-3 rounded-lg font-semibold hover:bg-orange-600 transition disabled:opacity-50 mb-3"
      >
    ...
    <Layout>
      {loading ? (
        ...
        <>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
              <div className="max-w-3xl">
                <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
                  ...
                  {/* Badge archived hiển thị trong hero */}
                  {isArchived && (
          ...
          {/* Banner cảnh báo course archived — chỉ hiện với student đã enrolled */}
          {isArchived && isEnrolled && (
            <div className="bg-amber-50 border-b border-amber-200">
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-2 text-amber-700 text-sm">
                ...
                Khóa học này đã ngưng xuất bản. Bạn vẫn có thể tiếp tục học vì đã đăng ký trước đó.
              </div>
            </div>
          )}
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-3 border-t pt-4">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">{course.lesson_count ?? 0} bài</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2 text-gray-600">
